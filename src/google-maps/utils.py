...
def format_weather_data(dataset: list[dict]) -> list[dict]:
    """
    Format and simplify the weather data by flattening the nested dict.

    Args:
        dataset (list): The weather dataset to be formatted.

    Returns:
        The simplified version of the dataset.
    """

    for _dataset in dataset:
        _dataset.update(**_dataset["interval"])
        _dataset.pop("interval")

        daytime_forecast = _dataset["daytimeForecast"]["weatherCondition"]["description"]
        night_time_forecast = _dataset["nighttimeForecast"]["weatherCondition"]["description"]

        max_temperature = _dataset['maxTemperature']
        min_temperature = _dataset['minTemperature']

        _dataset.update({
            "daytimeForecast": daytime_forecast["text"],
            "nighttimeForecast": night_time_forecast["text"],
            "maxTemperature": f"{max_temperature['degrees']} {max_temperature['unit']}",
            "minTemperature": f"{min_temperature['degrees']} {min_temperature['unit']}",
        })
